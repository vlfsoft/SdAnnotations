...
public @interface StructuralPattern {
     ...
     * an alternative naming shared with the {@link Adapter} pattern) is a design pattern
    ...
    @interface Association {
         ...
         * In {@link Composition}, when the owning object is destroyed, so are the contained objects. In {@link Aggregation}, this is not necessarily true.
         * See <a href="https://dzone.com/articles/aggregate-pattern">Aggregate Pattern</a>
         * {@link Aggregation} is a kind of {@link Association} that specifies a whole/part relationship between the aggregate (whole) and component part.
     ...
     * If you are only pulling items from a {@link Collection.Generic}, it is a {@link Producer} and you should use extends;
     * if you are only stuffing items in, it is a {@link Consumer} and you should use super. If you do both with the same {@link Collection.Generic},
    ...
    /**
     * Private method that is used locally in one (as a rule) or more (rarely) other class methods.
     * In kotlin - local function.
     * In java is implemented with either {@link LocalMethodGroup} or with {@link LocalMethodLambda}
     */
    @DesignPattern.Structural
    @Documented
    @Inherited
    ...
    /**
     * In java, this is a nested static class, that encapsulates in its namespace a group of private static methods (to support {@link LocalMethod}), thus excluding them from namespace of enclosed class.
     ...
     * As a rule, method with name execute is an {@link EntryPointMethod}.
     *
     * In kotlin, this is companion object. Methods can be annotated with @JvmStatic.
     *
    ...
    /**
     * Lambda that is stored in a local variable, to support {@link LocalMethod}
     */
    @LocalMethod
    @DesignPattern.Structural
    @Documented
